...
def query_expenditure(
    session: sa.orm.Session, measure_type: str
) -> tuple[pd.DataFrame, pd.DataFrame]:
    df_aus = pd.read_sql(
        session.query(crud.Expenditure)
        .join(crud.Country)
        .filter(
            crud.Country.name == "Australia",
            crud.Expenditure.measure_type == measure_type,
        )
        .statement,
        session.bind,
    )
...
vis.plot.plot_milex(
    ax_gov,
    "share_of_gov_spending",
    *query_expenditure(session, "share_of_gov_spending"),
)
...
# %%
filename = "images/milex_australia.svg"
fig.export(filename)
# %% Transfer data
...
query = query_core.join(crud.Country, crud.Transfer.supplier == crud.Country.id).filter(
    crud.Country.name == "Australia"
)
df_supplier = pd.read_sql(query.statement, session.bind)

query = query_core.join(
    crud.Country, crud.Transfer.recipient == crud.Country.id
).filter(crud.Country.name == "Australia")
df_recipient = pd.read_sql(query.statement, session.bind)
# %%
fig.clear()
