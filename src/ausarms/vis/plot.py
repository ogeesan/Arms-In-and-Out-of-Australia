...
def plot_milex(
    ...
    ax.plot(
        aus_data.year,
        aus_data.expenditure,
        label="Australia",
        color=palette.rgb("Australia"),
    )
    ...
    if fill == "std":
        ...
        mad_per_year = data.apply(lambda x: (x - x.median()).abs().median())
        err = mad_per_year
    else:
        err = 0
    ax.plot(
...
def calculate_decade_mean(yearly_total: pd.Series) -> tuple[list[int], list[float]]:
    ...
    for year in years:
        decade = (
            yearly_total[
                (yearly_total.index >= year) & (yearly_total.index < year + 10)
            ].sum()
            / 10
        )
